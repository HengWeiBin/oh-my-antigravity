...
This skill was originally designed for OpenCode/Codex harnesses. In Google Antigravity, we do not call `spawn_agent`, `multi_agent_v1.spawn_agent`, or `call_omo_agent(...)` literally. Translate all child agent operations to Antigravity native tools:

| Legacy example | Antigravity tool to use |
| --- | --- |
| `call_omo_agent(subagent_type="explore", ...)` | `invoke_subagent({ Subagents: [{ TypeName: "explore", Role: "Codebase Researcher", Prompt: "..." }] })` |
| `call_omo_agent(subagent_type="librarian", ...)` | `invoke_subagent({ Subagents: [{ TypeName: "librarian", Role: "Documentation Searcher", Prompt: "..." }] })` |
| `task(subagent_type="plan", ...)` | `invoke_subagent({ Subagents: [{ TypeName: "self", Role: "Planner", Prompt: "..." }] })` |
| `task(category="...", ...)` | `invoke_subagent({ Subagents: [{ TypeName: "self", Role: "Task Executor", Prompt: "..." }] })` |
| `background_output(task_id="...")` or `wait_agent(...)` | Not needed. Antigravity runs subagents asynchronously. Stop calling tools to end your turn, and you will be notified when the subagent replies. |
| `team_*(...)` | Define a list of subagents in `invoke_subagent({ Subagents: [...] })` to run them concurrently in the background. |

### Subagent Communication & Management
- **Sending messages**: Use `send_message({ Recipient: "<conversation-id>", Message: "..." })` to interact with an active subagent.
- **Managing subagents**: Use `manage_subagents({ Action: "list" })` to see active agents, or `Action: "kill"` to terminate them.
- **Managing background tasks**: Use `manage_task({ Action: "status", TaskId: "..." })` or similar to manage shell commands that run in the background.

When translating `load_skills=[...]`, specify the requested skill names in the spawned agent's `Prompt`.

For work likely to exceed one turn, require the child to report progress or status periodically. Stop calling tools to yield execution while waiting for subagents or background tasks to finish.

## Antigravity Subagent Reliability

Every `invoke_subagent` prompt is a self-contained executable assignment: `TASK: <imperative assignment>`, then `DELIVERABLE`, `SCOPE`, and `VERIFY`, with role instructions inside `Prompt`. Use `Workspace: "inherit"` (or omit to branch) depending on isolation needs; pass only the context the child needs.

Plan and reviewer agents may run for a long time: spawn them in the background and keep doing independent root work. Stop calling tools to yield execution; Antigravity will notify you when subagents finish. Require `WORKING: <task> - <current phase>` before long passes and `BLOCKED: <reason>` only when progress stops. Keep the parent visibly alive with active subagent count, names, and latest `WORKING:` phase. Fallback only when the child is completed without the deliverable, ack-only after followup, explicitly `BLOCKED:`, or no longer running — then record inconclusive (never a pass), close if safe, and respawn a smaller task with the missing deliverable.
...
6. **DELEGATE EVERYTHING. YOU NEVER IMPLEMENT.** Dispatch ALL independent sub-tasks across those checkboxes in one parallel `invoke_subagent` burst; serialize only named dependencies. Verification and checkbox marking stay per-checkbox.
...
5. One Manual-QA channel, named with the exact tool and exact invocation (the literal `curl`, `send-keys`, `browser:control-in-app-browser` action, `page.click`, payload, selectors, and the binary observable that decides PASS/FAIL), not "verify it works". A LIGHT checkbox needs one real-surface proof of its deliverable, and auxiliary surfaces (CLI stdout, DB state diff, parsed config dump) are first-class when the surface is CLI- or data-shaped:
   ...
   - TUI visual evidence: when a TUI claim needs visual QA or PR proof, run `node "<skill-root>/../visual-qa/scripts/web-terminal-visual-qa.mjs" --command "<cmd>" --input "{Enter}" --evidence-dir <dir>` (real pty rendered through xterm.js in Chrome) and attach `terminal.png` plus `metadata.json`.
