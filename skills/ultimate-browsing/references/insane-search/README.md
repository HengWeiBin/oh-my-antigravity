...
> The engine package it drives is at [../../engine/](../../engine/), invoked from the skill directory as `python -m engine "<URL>"`.
...
3. 식별된 JSON API URL을 `python -m engine <API_URL>`로 재호출 (백그라운드 engine과는 별개 호출). 대부분 API 레이어는 페이지 HTML보다 WAF 보호가 얕아 curl_cffi로 바로 수집됨
...
- **단일 진입점**: 일반 웹 페이지는 항상 `python -m engine <URL>` 또는 `from engine import fetch; fetch(...)`.
...
```bash
python -c "import curl_cffi, bs4, yaml" 2>/dev/null || pip install curl_cffi beautifulsoup4 pyyaml -q
```
