...
## Delegation (Antigravity-native)

Fan out read-only research before deciding. Every spawn names DELIVERABLE / SCOPE / VERIFY inside `Prompt`, states the role inside `Prompt`, and does not pass any history (omit `Workspace` to branch, or use `Workspace: "inherit"` if shared workspace is required):

```json
invoke_subagent({ Subagents: [{ TypeName: "explore", Role: "Codebase Researcher", Prompt: "TASK: act as an explorer. DELIVERABLE: ... SCOPE: ... VERIFY: ..." }] })
```

Spawn every independent child for the current wave first. After the wave is launched, stop calling tools to yield execution. Antigravity will notify you when subagents finish. Do not start dependent planning, drafting, approval-gate work, or final handoff until each child result is integrated or recorded as inconclusive.

Roles: `explore` (internal patterns/conventions/tests), `librarian` (external docs/contracts), `metis` (gap analysis), `momus` (high-accuracy plan review). Full spawn/wait/fallback discipline is in `references/full-workflow.md`.
