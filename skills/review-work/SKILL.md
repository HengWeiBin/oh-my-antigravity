...
- **Sending messages**: Use `send_message({ Recipient: "<conversation-id>", Message: "..." })` to interact with an active subagent (instead of `send_input` or `send_message` with task name).
...
Every `invoke_subagent` prompt must be self-contained. Start with
...
Use `Workspace: "inherit"` (or omit to branch) depending on isolation needs.
...
spawn scoped to the delta plus current evidence, never a follow-up
to a long-lived reviewer carrying stale context.

Plan and reviewer agents may run for a long time; spawn them in the background and keep doing independent root work. Stop calling tools to yield execution; Antigravity will notify you when subagents finish.
...
subagent count, agent names, and latest `WORKING:` phase. Track spawned agent names
locally. Antigravity will automatically notify you of updates when a subagent replies.
Treat a running child as alive. Fallback only when the child is
...
As each completes, collect its substantive final result. Preserve completed lane
