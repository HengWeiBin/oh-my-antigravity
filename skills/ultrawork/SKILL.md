...
Track spawned agent names locally. Antigravity will notify you of updates automatically when a subagent replies.
...
is launched, wait for each spawned child to reach terminal status
before any dependent `update_plan` transition, implementation tool call, plan
drafting, approval-gate work, PR handoff, or final response.
Do not write the final answer, PR handoff, or completion summary while
active child agents remain open. Yield execution (stop calling tools) and let the Antigravity system notify you when subagents update. If a subagent is stuck or silent for a long time, you may check on it by sending a query message or killing it via `manage_subagents` and respawn if necessary.
...
1. Spawn a child subagent with a self-contained reviewer assignment in `Prompt` using `invoke_subagent`. Specify the role clearly inside the `Prompt`.
